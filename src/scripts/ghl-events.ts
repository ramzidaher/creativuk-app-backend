...
async function fetchEvents() {
  ...
  const groupId = process.env.GHL_GROUP_ID;
  ...
  if (!calendarId && !userId && !groupId) {
    console.error('Missing GHL_CALENDAR_ID, GHL_USER_ID, or GHL_GROUP_ID in .env');
    process.exit(1);
  }
  ...
  try {
    ...
    const outputPath = path.join(outputDir, `ghl-calendar-events-${range.label}.json`);
    ...
    console.error('Failed to fetch events.', { status, code, message, data });
    process.exit(1);
  }
}
