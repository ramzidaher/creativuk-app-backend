...
function isCellVisuallyEnabled(cell) {
    if (!cell) {
    ...
    // Check cell style for enabled indicators
    if (cell.s) {
        // Check for yellow background (enabled input field)
        if (cell.s.fill && cell.s.fill.fgColor && cell.s.fill.fgColor.rgb === 'FFFF00') {
            return { enabled: true, reason: 'Yellow background (enabled input)' };
        }
        
        // Check for light green background (enabled input field)
        if (cell.s.fill && cell.s.fill.fgColor && cell.s.fill.fgColor.rgb === 'E3F1CB') {
            return { enabled: true, reason: 'Light green background (enabled input)' };
        }
        
        // Check for white background (enabled input field)
        if (cell.s.fill && cell.s.fill.fgColor && cell.s.fill.fgColor.rgb === 'FFFFFF') {
            return { enabled: true, reason: 'White background (enabled input)' };
        }
        
        // Check for dark gray background (disabled)
        if (cell.s.fill && cell.s.fill.fgColor && cell.s.fill.fgColor.rgb === '595959') {
            return { enabled: false, reason: 'Dark gray background (disabled)' };
        }
        
        // Check for light gray background (output/calculated field)
        if (cell.s.fill && cell.s.fill.fgColor && cell.s.fill.fgColor.rgb === 'F2F2F2') {
            return { enabled: false, reason: 'Light gray background (output field)' };
        }
    }
...
function findRadioButtonSelections(inputsSheet) {
    console.log('\n🎛️ FINDING RADIO BUTTON SELECTIONS:');
    console.log('====================================');
    
    const radioButtonSelections = {};
    
    // Look for radio button cells in column B and other columns
    ...
    for (let row = sheetRange.s.r; row <= sheetRange.e.r; row++) {
        for (let col = sheetRange.s.c; col <= sheetRange.e.c; col++) {
            ...
            if (cell && cell.v) {
                // Look for cells that might be radio button selections
                const value = cell.v.toString().toLowerCase();
                
                if (value === 'yes' || value === 'no' || 
                    value === 'single rate / standard' || value === 'dual rate / economy 7' ||
                    value === 'solar' || value === 'solar / hybrid inverter' || value === 'battery inverter' ||
                    value === 'self-consumption battery' || value === 'overnight charging battery' || value === 'no battery' ||
                    value === 'cash' || value === 'hometree' || value === 'finance(new)') {
                    
                    console.log(`${cellAddress}: "${cell.v}"`);
                    radioButtonSelections[cellAddress] = cell.v;
                }
            }
        }
    }
...
function getVisualDynamicInputs(excelFilePath = EXCEL_FILE_PATH) {
    console.log('🔍 Getting VISUAL Dynamic Inputs from Excel');
    console.log('==========================================');
    ...
    try {
        ...
        // Find radio button selections
        const radioButtonSelections = findRadioButtonSelections(inputsSheet);
        console.log('Radio button selections:', radioButtonSelections);
        
        // Analyze all input fields based on visual appearance
        console.log(`\n🔍 Analyzing ${ALL_INPUT_FIELDS.length} input fields visually...`);
        console.log('================================================');
        ...
        for (const field of ALL_INPUT_FIELDS) {
            ...
            const enabledStatus = isCellVisuallyEnabled(cell, field.cell);
            ...
            const result = {
                ...
                backgroundColor: cell && cell.s && cell.s.fill && cell.s.fill.fgColor ? cell.s.fill.fgColor.rgb : null
            };
        ...
        const outputPath = path.join(__dirname, 'visual-excel-inputs-results.json');
        ...
        console.error('❌ Error getting visual dynamic inputs:', error);
        return {
            success: false,
...
if (require.main === module) {
    getVisualDynamicInputs();
}

module.exports = { getVisualDynamicInputs };
